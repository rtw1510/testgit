...
@Configuration
public class EnSecAcctDelJobConfig {
    @Autowired
    ...
    @Autowired
    @Qualifier("EnSecAcctDelProcessor")
    ...
    @Autowired
    private OracleIbatisPagingItemReader enSecAcctDelReader;

    @Autowired
    private EntJobExecutionListener entJobExecutionListener;
    private int commit_interval = 1000;

    @Bean("EnSecAcctDelJob")
    public Job enSecAcctDelJob(){
        return jobBuilderFactory.get("EnSecAcctDelJob")
                .start(enSecAcctDelFlow())
                .end()
                .listener(entJobExecutionListener)
                .build();
    }
    @Bean
    public Flow enSecAcctDelFlow() {
        return new FlowBuilder<Flow>("EnSecAcctDelFlow")
                .start(enSecAcctDelStep())
                .build();
    }

    @Bean
    public Step enSecAcctDelStep() {
        return stepBuilderFactory.get("EnSecAcctDelStep")
                .partitioner(enSecAcctDelSonStep().getName(),entRangePartitioner)
                .partitionHandler(enSecAcctDelRangePartitioner())
                .build();
    }
    @Bean
    public PartitionHandler enSecAcctDelRangePartitioner() {
        TaskExecutorPartitionHandler handler = new TaskExecutorPartitionHandler();
        //handler.setGridSize(4);
        handler.setTaskExecutor( enSecAcctDelPartitionHandlerTaskExecutor());
        handler.setStep(enSecAcctDelSonStep());//暂时存在
        try {
    ...
    @Bean
    public SimpleAsyncTaskExecutor  enSecAcctDelPartitionHandlerTaskExecutor() {
        return new SimpleAsyncTaskExecutor();
    }
    @Bean
    public Step enSecAcctDelSonStep() {
        return stepBuilderFactory.get("EnSecAcctDelSon")
                .<EnSecAcctDel,EnSecAcctDel>chunk(commit_interval)
                .reader(enSecAcctDelReader)
