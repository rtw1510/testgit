...
@Configuration
public class EnAcctInfJobConfig {
    @Autowired
    ...
    @Autowired
    private OracleIbatisPagingItemReader enAcctInfReader;
    ...
    @Bean("EnAcctInfJob")
    public Job enAcctInfJob(){
        return jobBuilderFactory.get("EnAcctInfJob")
                .start(enAcctInfFlow())
                .end()
    ...
    @Bean
    public Flow enAcctInfFlow() {
        return new FlowBuilder<Flow>("EnAcctInfFlow")
                .start(enAcctInfStep())
                .build();
    }

    @Bean
    public Step enAcctInfStep() {
        return stepBuilderFactory.get("EnAcctInfStep")
                .partitioner(enAcctInfSonStep().getName(),entRangePartitioner)
                .partitionHandler(enAcctInfRangePartitioner())
                .build();
    }
    @Bean
    public PartitionHandler enAcctInfRangePartitioner() {
        TaskExecutorPartitionHandler handler = new TaskExecutorPartitionHandler();
        //handler.setGridSize(4);
        handler.setTaskExecutor( enAcctInfPartitionHandlerTaskExecutor());
        handler.setStep(enAcctInfSonStep());//暂时存在
        try {
    ...
    @Bean
    public SimpleAsyncTaskExecutor  enAcctInfPartitionHandlerTaskExecutor() {
        return new SimpleAsyncTaskExecutor();
    }
    @Bean
    public Step enAcctInfSonStep() {
        return stepBuilderFactory.get("EnAcctInfSon")
                .<Eb_CorpAcct_Info,Eb_CorpAcct_Info>chunk(commit_interval)
                .reader(enAcctInfReader)
