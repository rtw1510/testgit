...
@Configuration
public class EntMotgaCltalCtrctInfJobConfig {
    @Autowired
    ...
    @Autowired
    @Qualifier("EntMotgaCltalCtrctInfProcessor")
    ...
    @Autowired
    private OracleIbatisPagingItemReader entMotgaCltalCtrctInfReader;

    @Autowired
    private EntJobExecutionListener entJobExecutionListener;
    private int commit_interval = 1000;

    @Bean("EntMotgaCltalCtrctInfJob")
    public Job entMotgaCltalCtrctInfJob(){
        return jobBuilderFactory.get("EntMotgaCltalCtrctInfJob")
                .start(entMotgaCltalCtrctInfFlow())
                .end()
                .listener(entJobExecutionListener)
                .build();
    }
    @Bean
    public Flow entMotgaCltalCtrctInfFlow() {
        return new FlowBuilder<Flow>("EntMotgaCltalCtrctInfFlow")
                .start(entMotgaCltalCtrctInfStep())
                .build();
    }

    @Bean
    public Step entMotgaCltalCtrctInfStep() {
        return stepBuilderFactory.get("EntMotgaCltalCtrctInfStep")
                .partitioner(entMotgaCltalCtrctInfSonStep().getName(),entRangePartitioner)
                .partitionHandler(entMotgaCltalCtrctInfRangePartitioner())
                .build();
    }
    @Bean
    public PartitionHandler entMotgaCltalCtrctInfRangePartitioner() {
        TaskExecutorPartitionHandler handler = new TaskExecutorPartitionHandler();
        //handler.setGridSize(4);
        handler.setTaskExecutor( entMotgaCltalCtrctInfPartitionHandlerTaskExecutor());
        handler.setStep(entMotgaCltalCtrctInfSonStep());//暂时存在
        try {
    ...
    @Bean
    public SimpleAsyncTaskExecutor  entMotgaCltalCtrctInfPartitionHandlerTaskExecutor() {
        return new SimpleAsyncTaskExecutor();
    }
    @Bean
    public Step entMotgaCltalCtrctInfSonStep() {
        return stepBuilderFactory.get("EntMotgaCltalCtrctInfSon")
                .<Eb_Guacct_Info,Eb_Guacct_Info>chunk(commit_interval)
                .reader(entMotgaCltalCtrctInfReader)
