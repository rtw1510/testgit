...
@Configuration
public class EnAcctInfEntDelJobConfig {
    @Autowired
    ...
    @Autowired
    @Qualifier("EnAcctInfEntDelProcessor")
    ...
    @Autowired
    private OracleIbatisPagingItemReader enAcctInfEntDelReader;

    @Autowired
    private EntJobExecutionListener entJobExecutionListener;
    private int commit_interval = 1000;

    @Bean("EnAcctInfEntDelJob")
    public Job enAcctInfEntDelJob(){
        return jobBuilderFactory.get("EnAcctInfEntDelJob")
                .start(enAcctInfEntDelFlow())
                .end()
                .listener(entJobExecutionListener)
                .build();
    }
    @Bean
    public Flow enAcctInfEntDelFlow() {
        return new FlowBuilder<Flow>("EnAcctInfEntDelFlow")
                .start(enAcctInfEntDelStep())
                .build();
    }

    @Bean
    public Step enAcctInfEntDelStep() {
        return stepBuilderFactory.get("EnAcctInfEntDelStep")
                .partitioner(enAcctInfEntDelSonStep().getName(),entRangePartitioner)
                .partitionHandler(enAcctInfEntDelRangePartitioner())
                .build();
    }
    @Bean
    public PartitionHandler enAcctInfEntDelRangePartitioner() {
        TaskExecutorPartitionHandler handler = new TaskExecutorPartitionHandler();
        //handler.setGridSize(4);
        handler.setTaskExecutor( enAcctInfEntDelPartitionHandlerTaskExecutor());
        handler.setStep(enAcctInfEntDelSonStep());//暂时存在
        try {
    ...
    @Bean
    public SimpleAsyncTaskExecutor  enAcctInfEntDelPartitionHandlerTaskExecutor() {
        return new SimpleAsyncTaskExecutor();
    }
    @Bean
    public Step enAcctInfEntDelSonStep() {
        return stepBuilderFactory.get("EnAcctInfEntDelSon")
                .<EnAcctInfEntDel,EnAcctInfEntDel>chunk(commit_interval)
                .reader(enAcctInfEntDelReader)
