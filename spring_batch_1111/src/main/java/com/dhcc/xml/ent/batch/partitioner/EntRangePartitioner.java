package com.dhcc.xml.ent.batch.partitioner;
...
@Component
@StepScope
public class EntRangePartitioner implements Partitioner {

	private static final Logger log = LoggerFactory.getLogger(EntRangePartitioner.class);
	...
	public Map<String, ExecutionContext> partition(int maxGridSize) {
       ...
       // int maxSum = 500000;
		...
		//设置Job循环执行的数量
		if(EntJobExecutionListener.JOB_MAP.get(jobName) == 0){
			EntJobExecutionListener.JOB_MAP.put(jobName,times);
		}
		if(EntJobExecutionListener.JOB_MAP.get(jobName)!=1){
			sum=maxSum;
		}else{
			sum=sum-(times-1)*maxSum;
		}
		//分区起始id
		int fromId = (times-EntJobExecutionListener.JOB_MAP.get(jobName))*maxSum+1;
