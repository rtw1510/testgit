...
@Configuration
public class EnCtrctInfDelJobConfig {
    @Autowired
    ...
    @Autowired
    @Qualifier("EnCtrctInfDelProcessor")
    ...
    @Autowired
    private OracleIbatisPagingItemReader enCtrctInfDelReader;

    @Autowired
    private EntJobExecutionListener entJobExecutionListener;
    private int commit_interval = 1000;

    @Bean("EnCtrctInfDelJob")
    public Job enCtrctInfDelJob(){
        return jobBuilderFactory.get("EnCtrctInfDelJob")
                .start(enCtrctInfDelFlow())
                .end()
                .listener(entJobExecutionListener)
                .build();
    }
    @Bean
    public Flow enCtrctInfDelFlow() {
        return new FlowBuilder<Flow>("EnCtrctInfDelFlow")
                .start(enCtrctInfDelStep())
                .build();
    }

    @Bean
    public Step enCtrctInfDelStep() {
        return stepBuilderFactory.get("EnCtrctInfDelStep")
                .partitioner(enCtrctInfDelSonStep().getName(),entRangePartitioner)
                .partitionHandler(enCtrctInfDelRangePartitioner())
                .build();
    }
    @Bean
    public PartitionHandler enCtrctInfDelRangePartitioner() {
        TaskExecutorPartitionHandler handler = new TaskExecutorPartitionHandler();
        //handler.setGridSize(4);
        handler.setTaskExecutor( enCtrctInfDelPartitionHandlerTaskExecutor());
        handler.setStep(enCtrctInfDelSonStep());//暂时存在
        try {
    ...
    @Bean
    public SimpleAsyncTaskExecutor  enCtrctInfDelPartitionHandlerTaskExecutor() {
        return new SimpleAsyncTaskExecutor();
    }
    @Bean
    public Step enCtrctInfDelSonStep() {
        return stepBuilderFactory.get("EnCtrctInfDelSon")
                .<EnCtrctInfDel,EnCtrctInfDel>chunk(commit_interval)
                .reader(enCtrctInfDelReader)
