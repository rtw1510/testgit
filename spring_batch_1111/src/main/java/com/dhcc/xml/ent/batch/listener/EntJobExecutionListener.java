package com.dhcc.xml.ent.batch.listener;
...
import com.dhcc.xml.ent.batch.launch.EntBatchJobExecute;
...
@Component
public class EntJobExecutionListener implements JobExecutionListener {
	...
	public void beforeJob(JobExecution jobExecution) {
		...
		//一个记录类型的开始时间
		if(EntJobExecutionListener.JOB_MAP.get(jobName)==null|| EntJobExecutionListener.JOB_MAP.get(jobName)==0){
			EntJobExecutionListener.JOB_MAP.put(jobName,0);
			/**start插入批量执行情况信息表*/
			...
			if (jobExecution.getStatus() == BatchStatus.COMPLETED) {
				try {
					...
					if(sum/maxSum==0){
						...
						formPageNumber = (jobSum - EntJobExecutionListener.JOB_MAP.get(jobName)) * maxSum + 1;
						if(EntJobExecutionListener.JOB_MAP.get(jobName)==1){//最后一次执行该Job
							toPageNumber = sum;
						}else{
							toPageNumber = (jobSum - EntJobExecutionListener.JOB_MAP.get(jobName) + 1) * maxSum;
						}
					}
					
					//执行成功当前Job-1
					EntJobExecutionListener.JOB_MAP.put(jobName, EntJobExecutionListener.JOB_MAP.get(jobName)-1);
					if(EntJobExecutionListener.JOB_MAP.get(jobName)>0){//还需要执行当前Job
					...
					if(batchJob!=null&& EntJobExecutionListener.JOB_MAP.get(jobName)>0){//还需要执行当前Job
						System.out.println("============="+ EntJobExecutionListener.JOB_MAP.get(jobName)+"=============");
...
//						messageInfoBo.updateRptType("MessageInfo.updateRptType", map);//更新主表上报状态和每个段FLAG标志
					...
					EntJobExecutionListener.JOB_MAP.put(jobName,0);//Job执行出现错误停止当前Job报文生成功能
					/**start更新批量执行情况信息表*/
					BatchExecuteInfo batchExecuteInfo = new BatchExecuteInfo();
					batchExecuteInfo.setJobId(jobName);
			...
			if (jobExecution.getStatus() == BatchStatus.COMPLETED) {
				try {
					...
					if(sum/maxSum==0){
						...
						formPageNumber = (EntJobExecutionListener.JOB_MAP.get(jobName)) * maxSum + 1;
						if(EntJobExecutionListener.JOB_MAP.get(jobName)==1){//最后一次执行该Job
							toPageNumber = sum;
						}else{
							toPageNumber = (jobSum - EntJobExecutionListener.JOB_MAP.get(jobName) + 1) * maxSum;
