...
@Configuration
public class EnSecAcctInfJobConfig {
    @Autowired
    ...
    @Autowired
    @Qualifier("EnSecAcctInfProcessor")
    ...
    @Autowired
    private OracleIbatisPagingItemReader enSecAcctInfReader;

    @Autowired
    private EntJobExecutionListener entJobExecutionListener;
    private int commit_interval = 1000;

    @Bean("EnSecAcctInfJob")
    public Job enSecAcctInfJob(){
        return jobBuilderFactory.get("EnSecAcctInfJob")
                .start(enSecAcctInfFlow())
                .end()
                .listener(entJobExecutionListener)
                .build();
    }
    @Bean
    public Flow enSecAcctInfFlow() {
        return new FlowBuilder<Flow>("EnSecAcctInfFlow")
                .start(enSecAcctInfStep())
                .build();
    }

    @Bean
    public Step enSecAcctInfStep() {
        return stepBuilderFactory.get("EnSecAcctInfStep")
                .partitioner(enSecAcctInfSonStep().getName(),entRangePartitioner)
                .partitionHandler(enSecAcctInfRangePartitioner())
                .build();
    }
    @Bean
    public PartitionHandler enSecAcctInfRangePartitioner() {
        TaskExecutorPartitionHandler handler = new TaskExecutorPartitionHandler();
        //handler.setGridSize(4);
        handler.setTaskExecutor( enSecAcctInfPartitionHandlerTaskExecutor());
        handler.setStep(enSecAcctInfSonStep());//暂时存在
        try {
    ...
    @Bean
    public SimpleAsyncTaskExecutor  enSecAcctInfPartitionHandlerTaskExecutor() {
        return new SimpleAsyncTaskExecutor();
    }
    @Bean
    public Step enSecAcctInfSonStep() {
        return stepBuilderFactory.get("EnSecAcctInfSon")
                .<Eb_GuarAcc_Info,Eb_GuarAcc_Info>chunk(commit_interval)
                .reader(enSecAcctInfReader)
