...
@Configuration
public class AcctBsSgmtJobConfig {
    @Autowired
    ...
    @Autowired
    private OracleIbatisPagingItemReader acctBsSgmtReader;
    ...
    @Bean("AcctBsSgmtJob")
    public Job acctBsSgmtJob(){
        return jobBuilderFactory.get("AcctBsSgmtJob")
                .start(acctBsSgmtFlow())
                .end()
                .listener(perJobExecutionListener)
                .build();
    }
    @Bean
    public Flow acctBsSgmtFlow() {
        return new FlowBuilder<Flow>("AcctBsSgmtFlow")
                .start(acctBsSgmtStep())
                .build();
    }

    @Bean
    public Step acctBsSgmtStep() {
        return stepBuilderFactory.get("AcctBsSgmtStep")
                .partitioner(AcctBsSgmtSonStep().getName(),perRangePartitioner)
                .partitionHandler(acctBsSgmtRangePartitioner())
                .build();
    }
    @Bean
    public PartitionHandler acctBsSgmtRangePartitioner() {
        TaskExecutorPartitionHandler handler = new TaskExecutorPartitionHandler();
        //handler.setGridSize(4);
        handler.setTaskExecutor( acctBsSgmtPartitionHandlerTaskExecutor());
        handler.setStep(AcctBsSgmtSonStep());//暂时存在
        try {
    ...
    @Bean
    public SimpleAsyncTaskExecutor  acctBsSgmtPartitionHandlerTaskExecutor() {
        return new SimpleAsyncTaskExecutor();
    }
    @Bean
    public Step AcctBsSgmtSonStep() {
        return stepBuilderFactory.get("AcctBsSgmtSon")
                .<Pb_Lnacct_Info,Pb_Lnacct_Info>chunk(commit_interval)
                .reader(acctBsSgmtReader)
