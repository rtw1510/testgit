...
@Configuration
public class InSecAcctInfJobConfig {
    @Autowired
    ...
    @Autowired
    @Qualifier("InSecAcctInfProcessor")
    ...
    @Autowired
    private OracleIbatisPagingItemReader inSecAcctInfReader;

    @Autowired
    private PerJobExecutionListener perJobExecutionListener;
    private int commit_interval = 1000;

    @Bean("InSecAcctInfJob")
    public Job inSecAcctInfJob(){
        return jobBuilderFactory.get("InSecAcctInfJob")
                .start(inSecAcctInfFlow())
                .end()
                .listener(perJobExecutionListener)
                .build();
    }
    @Bean
    public Flow inSecAcctInfFlow() {
        return new FlowBuilder<Flow>("InSecAcctInfFlow")
                .start(inSecAcctInfStep())
                .build();
    }

    @Bean
    public Step inSecAcctInfStep() {
        return stepBuilderFactory.get("InSecAcctInfStep")
                .partitioner(InSecAcctInfSonStep().getName(),perRangePartitioner)
                .partitionHandler(inSecAcctInfRangePartitioner())
                .build();
    }
    @Bean
    public PartitionHandler inSecAcctInfRangePartitioner() {
        TaskExecutorPartitionHandler handler = new TaskExecutorPartitionHandler();
        //handler.setGridSize(4);
        handler.setTaskExecutor( inSecAcctInfPartitionHandlerTaskExecutor());
        handler.setStep(InSecAcctInfSonStep());//暂时存在
        try {
    ...
    @Bean
    public SimpleAsyncTaskExecutor  inSecAcctInfPartitionHandlerTaskExecutor() {
        return new SimpleAsyncTaskExecutor();
    }
    @Bean
    public Step InSecAcctInfSonStep() {
        return stepBuilderFactory.get("InSecAcctInfSon")
                .<Pb_LnsecAcct_Info,Pb_LnsecAcct_Info>chunk(commit_interval)
                .reader(inSecAcctInfReader)
