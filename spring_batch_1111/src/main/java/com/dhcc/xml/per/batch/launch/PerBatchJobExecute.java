package com.dhcc.xml.per.batch.launch;
 ...
 */
@Component
public class PerBatchJobExecute {
	...
	public String run(String job_name,String br_no, String tx_date,String type) {
		...
		try {
			...
			String sum = "";
			if(PerJobExecutionListener.JOB_MAP.get(job_name)==null||PerJobExecutionListener.JOB_MAP.get(job_name)==0){
			...
			//报文生成的路径
			String filePath = dbLaunchService.getPath("S_PATH_INFO_getPath", "MESSAGEINFO_PER_PATH")+currenDate+"/";
			...
			JobExecution run =
				...
				PerJobExecutionListener.JOB_MAP.put(job_name,0);
				/**start更新批量执行情况信息表*/
