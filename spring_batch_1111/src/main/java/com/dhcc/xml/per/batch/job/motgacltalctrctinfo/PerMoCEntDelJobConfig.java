...
@Configuration
public class PerMoCEntDelJobConfig {
    @Autowired
    ...
    @Autowired
    @Qualifier("PerMoCEntDel_Processor")
    ...
    @Autowired
    private OracleIbatisPagingItemReader perMoCEntDelReader;

    @Autowired
    private PerJobExecutionListener perJobExecutionListener;
    private int commit_interval = 1000;

    @Bean("PerMoCEntDelJob")
    public Job perMoCEntDelJob(){
        return jobBuilderFactory.get("PerMoCEntDelJob")
                .start(perMoCEntDelFlow())
                .end()
                .listener(perJobExecutionListener)
                .build();
    }
    @Bean
    public Flow perMoCEntDelFlow() {
        return new FlowBuilder<Flow>("PerMoCEntDelFlow")
                .start(perMoCEntDelStep())
                .build();
    }

    @Bean
    public Step perMoCEntDelStep() {
        return stepBuilderFactory.get("PerMoCEntDelStep")
                .partitioner(perMoCEntDelSonStep().getName(),perRangePartitioner)
                .partitionHandler(perMoCEntDelRangePartitioner())
                .build();
    }
    @Bean
    public PartitionHandler perMoCEntDelRangePartitioner() {
        TaskExecutorPartitionHandler handler = new TaskExecutorPartitionHandler();
        //handler.setGridSize(4);
        handler.setTaskExecutor( perMoCEntDelPartitionHandlerTaskExecutor());
        handler.setStep(perMoCEntDelSonStep());//暂时存在
        try {
    ...
    @Bean
    public SimpleAsyncTaskExecutor  perMoCEntDelPartitionHandlerTaskExecutor() {
        return new SimpleAsyncTaskExecutor();
    }
    @Bean
    public Step perMoCEntDelSonStep() {
        return stepBuilderFactory.get("PerMoCEntDelSon")
                .<MoCEntDel,MoCEntDel>chunk(commit_interval)
                .reader(perMoCEntDelReader)
