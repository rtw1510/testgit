...
@Configuration
public class PerMotgaCltalCtrctInfJobConfig {
    @Autowired
    ...
    @Autowired
    @Qualifier("PerMotgaCltalCtrctInf_Processor")
    ...
    @Autowired
    private OracleIbatisPagingItemReader perMotgaCltalCtrctInfReader;

    @Autowired
    private PerJobExecutionListener perJobExecutionListener;
    private int commit_interval = 1000;

    @Bean("PerMotgaCltalCtrctInfJob")
    public Job perMotgaCltalCtrctInfJob(){
        return jobBuilderFactory.get("PerMotgaCltalCtrctInfJob")
                .start(perMotgaCltalCtrctInfFlow())
                .end()
                .listener(perJobExecutionListener)
                .build();
    }
    @Bean
    public Flow perMotgaCltalCtrctInfFlow() {
        return new FlowBuilder<Flow>("PerMotgaCltalCtrctInfFlow")
                .start(perMotgaCltalCtrctInfStep())
                .build();
    }

    @Bean
    public Step perMotgaCltalCtrctInfStep() {
        return stepBuilderFactory.get("PerMotgaCltalCtrctInfStep")
                .partitioner(PerMotgaCltalCtrctInfSonStep().getName(),perRangePartitioner)
                .partitionHandler(perMotgaCltalCtrctInfRangePartitioner())
                .build();
    }
    @Bean
    public PartitionHandler perMotgaCltalCtrctInfRangePartitioner() {
        TaskExecutorPartitionHandler handler = new TaskExecutorPartitionHandler();
        //handler.setGridSize(4);
        handler.setTaskExecutor( perMotgaCltalCtrctInfPartitionHandlerTaskExecutor());
        handler.setStep(PerMotgaCltalCtrctInfSonStep());//暂时存在
        try {
    ...
    @Bean
    public SimpleAsyncTaskExecutor  perMotgaCltalCtrctInfPartitionHandlerTaskExecutor() {
        return new SimpleAsyncTaskExecutor();
    }
    @Bean
    public Step PerMotgaCltalCtrctInfSonStep() {
        return stepBuilderFactory.get("PerMotgaCltalCtrctInfSon")
                .<Pb_Guacct_Info,Pb_Guacct_Info>chunk(commit_interval)
                .reader(perMotgaCltalCtrctInfReader)
