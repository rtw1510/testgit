...
@Configuration
public class InSecAcctDelJobConfig {
    @Autowired
    ...
    @Autowired
    @Qualifier("InSecAcctDelProcessor")
    ...
    @Autowired
    private OracleIbatisPagingItemReader inSecAcctDelReader;

    @Autowired
    private PerJobExecutionListener perJobExecutionListener;
    private int commit_interval = 1000;

    @Bean("InSecAcctDelJob")
    public Job inSecAcctDelJob(){
        return jobBuilderFactory.get("InSecAcctDelJob")
                .start(inSecAcctDelFlow())
                .end()
                .listener(perJobExecutionListener)
                .build();
    }
    @Bean
    public Flow inSecAcctDelFlow() {
        return new FlowBuilder<Flow>("InSecAcctDelFlow")
                .start(inSecAcctDelStep())
                .build();
    }

    @Bean
    public Step inSecAcctDelStep() {
        return stepBuilderFactory.get("InSecAcctDelStep")
                .partitioner(inSecAcctDelSonStep().getName(),perRangePartitioner)
                .partitionHandler(inSecAcctDelRangePartitioner())
                .build();
    }
    @Bean
    public PartitionHandler inSecAcctDelRangePartitioner() {
        TaskExecutorPartitionHandler handler = new TaskExecutorPartitionHandler();
        //handler.setGridSize(4);
        handler.setTaskExecutor( inSecAcctDelPartitionHandlerTaskExecutor());
        handler.setStep(inSecAcctDelSonStep());//暂时存在
        try {
    ...
    @Bean
    public SimpleAsyncTaskExecutor  inSecAcctDelPartitionHandlerTaskExecutor() {
        return new SimpleAsyncTaskExecutor();
    }
    @Bean
    public Step inSecAcctDelSonStep() {
        return stepBuilderFactory.get("InSecAcctDelSon")
                .<InSecAcctDel,InSecAcctDel>chunk(commit_interval)
                .reader(inSecAcctDelReader)
